...
public class ResourceManager
{
	...
	public Customer checkCustomer(int xid, int customerID) throws RemoteException
	{
		// Read customer object if it exists (and read lock it)
		Customer customer = (Customer)readData(xid, Customer.getKey(customerID));
		if (customer == null)
	...
	// Check if item exist
	public ReservableItem checkItem(int xid, int customerID, String key, String location) throws RemoteException
	{
		ReservableItem item = (ReservableItem)readData(xid, key);
		if (item == null)
		{
	...
	// Modify the customer after an item reservation
	public void customerReserve(int xid, Customer customer, String key, String location, ReservableItem item) throws RemoteException
	{
		customer.reserve(key, location, item.getPrice());
		writeData(xid, customer.getKey(), customer);
	}
	...
	// Decrease the number of available items in the storage
	public void itemReserve(int xid, ReservableItem item) throws RemoteException
	{
		item.setCount(item.getCount() - 1);
		item.setReserved(item.getReserved() + 1);
		writeData(xid, item.getKey(), item);
